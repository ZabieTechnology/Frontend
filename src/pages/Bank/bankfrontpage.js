...
const BankAccountCard = ({ bankName, accountNumber, balance, statementDate }) => {
  const cardStyle = {
    backgroundColor: '#FFFDE7', padding: { xs: '16px', sm: '24px' }, marginBottom: '24px',
  ...
  return (
    <Paper sx={cardStyle}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: { xs: 1.5, sm: 2 } }}>
        ...
        <Button variant="contained" sx={actionButtonStyle}> Manage Account </Button>
      </Box>
      <Typography variant="h4" component="p" sx={{ color: '#263238', mb: 0.5 }}> ₹ {balance} </Typography>
      <Typography variant="body2" sx={{ color: 'text.secondary', mb: { xs: 2.5, sm: 3 } }}> Statement Balance as of {statementDate} </Typography>
      <Button variant="contained" sx={actionButtonStyle}> Reconcile </Button>
    </Paper>
  );
};
      ...
      <Grid container spacing={4}>
        ...
        <Grid item xs={12} md={5}>
          <Paper sx={formSectionPaperStyle}>
            ...
            <Stack spacing={2.5}>
              <TextField fullWidth label="Bank name" variant="outlined" /> <TextField fullWidth label="Account name" variant="outlined" />
              <TextField fullWidth label="Account type" variant="outlined" /> <TextField fullWidth label="Currency" variant="outlined" />
                ...
                <Grid container spacing={2}>
                    <Grid item xs={12} sm={6}>
                        <FormControl fullWidth>
                            ...
                            <Select value={bankAccount} label="Select Bank Account" onChange={(e) => setBankAccount(e.target.value)}>
...
function App() {
  ...
  const renderContent = () => {
    if (currentPage === 'addAccount') return <AddBankAccountPage onBack={() => setCurrentPage('dashboard')} />;
    if (currentPage === 'addRules') return <AddBankRulesPage onBack={() => setCurrentPage('dashboard')} />;
    return (
      <>
        <Stack direction="row" spacing={2} sx={{ mb: { xs: 3, sm: 4 } }} >
          <Button variant="contained" sx={actionButtonStyle} onClick={() => setCurrentPage('addAccount')}> + Add Bank Account </Button>
          <Button variant="contained" sx={actionButtonStyle} onClick={() => setCurrentPage('addRules')}> + Add Bank Rules </Button>
        </Stack>
